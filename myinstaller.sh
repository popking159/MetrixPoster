...
# wget -qO - https://raw.githubusercontent.com/popking159/MetrixPoster/refs/heads/main/myinstaller.sh | /bin/sh
...
# 0. System Compatibility Checks
log "[INFO] Checking system compatibility..."

# Check if the image is OpenATV
if ! grep -iq "openatv" /etc/issue /etc/image-version /etc/os-release 2>/dev/null; then
    log "[ERROR] This plugin is specifically designed for OpenATV images."
    log "[ERROR] Incompatible image detected. Aborting installation."
    exit 1
fi

# Check if the MetrixHD skin directory exists
if [ ! -d "/usr/share/enigma2/MetrixHD/" ]; then
    log "[ERROR] MetrixHD skin directory (/usr/share/enigma2/MetrixHD/) not found."
    log "[ERROR] This plugin requires the MetrixHD skin. Aborting installation."
    exit 1
fi

log "[OK] System compatibility verified (OpenATV + MetrixHD)."
